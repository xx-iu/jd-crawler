...
class JDCommentsCrawler:
    ...
    def crawler(self):
        ...
        for p in range(self.page):
            ...
            for com in comments:
                tmp_list.append(
                    [com['content'], com['creationTime'],\
                     com['referenceTime'], com['score'],com['nickname']])
            df = pd.DataFrame(tmp_list,
                              columns=['content', 'create_time','reference_time', 'score','nickname'])
            self.save_csv(df, p)
            dfs.append(df)
